...
export default class CameraScreen extends React.Component {
  ...
  onPictureSaved = async photo => {
    var newImgPath = `${FileSystem.documentDirectory}photos/${Date.now()}.jpg`;
    await FileSystem.moveAsync({
      from: photo.uri,
      to: newImgPath,
    });
    this.setState({ newPhotos: true, lastTookImg: newImgPath });
  }
        ...
        <Dialog width={1}
          ...
          footer={
            <DialogFooter>
              <DialogButton
                text="Descartar"
                onPress={() => {this.takeOther()}}
              />
              <DialogButton
                text="Seleccionar"
                onPress={() => {this.saveAndReturn(callFrom)}}
              />
            </DialogFooter>
          }
        >
  ...
  renderCamera = (callFrom) =>
    (
      <View style={{ flex: 1 }}>

        {this.renderPreview(callFrom)}
  ...
  render() {
    ...
    const callFrom = navigation.getParam('callFrom', 'I dont know');

    const cameraScreenContent = this.state.permissionsGranted
      ? this.renderCamera(callFrom)
